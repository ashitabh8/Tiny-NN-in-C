...
class DynamicQuantRuleMinMaxPerChannel(QuantRule):
    """
    Dynamic quantization using min-max per-channel (per output-channel).

    Each output channel gets its own scale, computed from that channel's
    weight slice.  Activations are still quantized per-tensor at runtime.
    """

    def __init__(self, pattern: str, dtype: str):
        super().__init__(pattern, dtype)
        self._computed_scales: Optional[np.ndarray] = None
        self._computed_offset: Optional[int] = None

    def create_quant_node(self, node):
        raise NotImplementedError(
            "DynamicQuantRuleMinMaxPerChannel.create_quant_node requires "
    ...
    def create_quant_node_with_weights(self, node, weights: np.ndarray):
        """Create a quantized node with per-channel scales computed from weights."""
        scales, offset = self._compute_per_channel_scale_offset(weights)
        self._computed_scales = scales
        self._computed_offset = offset

        if node.op_type == 'linear':
            # dense_int8 kernel only supports a single weight scale, so we
            # consolidate per-channel scales to max (safe upper bound).
            from .ops.quant_linear import StaticQuantLinearNode
            scalar_scale = float(np.max(scales))
            return StaticQuantLinearNode(
                original_node=node,
                dtype=self.dtype,
                input_scale=scalar_scale,
                weight_scale=scalar_scale,
                output_scale=scalar_scale,
                offset=offset,
            )
        elif node.op_type == 'conv2d':
            from .ops.quant_conv2d import StaticQuantConv2dNode
            return StaticQuantConv2dNode(
                original_node=node,
                dtype=self.dtype,
                input_scale=float(np.mean(scales)),
                weight_scale=scales,
                output_scale=float(np.mean(scales)),
                offset=offset,
            )
        else:
    ...
    def quantize_weights(self, weights: np.ndarray) -> np.ndarray:
        scales, offset = self._compute_per_channel_scale_offset(weights)
        scale_shape = [1] * (weights.ndim - 1) + [scales.shape[0]]
        weights_q = np.round(weights / scales.reshape(scale_shape)) + offset
        if self.dtype == 'int8':
            weights_q = np.clip(weights_q, -128, 127).astype(np.int8)
        elif self.dtype == 'int16':
            weights_q = np.clip(weights_q, -32768, 32767).astype(np.int16)
    ...
    def _compute_per_channel_scale_offset(self, weights: np.ndarray) -> tuple:
        """Compute per-output-channel symmetric scale.

        For conv2d weights in HWIO layout the last axis is out_channels.
        For linear weights in [in, out] layout the last axis is out_features.
        """
        if self.dtype == 'int8':
            q_max = 127
        elif self.dtype == 'int16':
        ...
        for c in range(num_channels):
            channel_slice = weights[..., c]
            absmax = max(abs(float(np.min(channel_slice))),
                         abs(float(np.max(channel_slice))))
            if absmax == 0:
                scales[c] = 1.0 / q_max
            else:
                scales[c] = (absmax * safety_factor) / q_max
    ...
    def get_quant_params(self) -> Dict[str, Any]:
        params: Dict[str, Any] = {
            'dtype': self.dtype,
            'strategy': 'dynamic_minmax_per_channel',
        }
        if self._computed_scales is not None:
            params['scales'] = self._computed_scales
            params['offset'] = self._computed_offset
        return params

    def __repr__(self) -> str:
        return f"DynamicQuantRuleMinMaxPerChannel(pattern='{self.pattern}', dtype='{self.dtype}')"
