...
void model_forward(const float* input, float* output) {
    ...
    relu(slot_1, 10752);
    // exit_conv [conv2d]
    conv2d_nhwc(slot_1, 7, 256, 6, exit_conv_weight, 5, 5, 6, NULL, 2, 2, 1, slot_0);
    // exit_bn [batchnorm]
    batchnorm2d_nhwc(slot_0, 4, 128, 6, exit_bn_gamma, exit_bn_beta, exit_bn_mean, exit_bn_var, 1e-05f, slot_1);
    // exit_relu [relu]
    relu(slot_1, 3072);
    // exit_pool [adaptive_avg_pool]
    global_average_pool_2d(slot_1, 4, 128, 6, slot_0);
    // view [method_view]
