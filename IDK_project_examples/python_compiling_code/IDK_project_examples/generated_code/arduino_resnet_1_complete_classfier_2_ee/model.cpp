...
void model_forward(const float* input, float* output) {
    ...
    relu(slot_0, 10752);
    // exit_conv [conv2d]
    conv2d_nhwc(slot_0, 7, 256, 6, exit_conv_weight, 3, 3, 6, NULL, 2, 2, 0, slot_1);
    // exit_bn [batchnorm]
    batchnorm2d_nhwc(slot_1, 3, 127, 6, exit_bn_gamma, exit_bn_beta, exit_bn_mean, exit_bn_var, 1e-05f, slot_0);
    // exit_relu [relu]
    relu(slot_0, 2286);
    // exit_pool [adaptive_avg_pool]
    global_average_pool_2d(slot_0, 3, 127, 6, slot_1);
    // view [method_view]
