...
void model_forward(const float* input, float* output) {
    ...
    _t_start = micros();
    static float slot_0[3584];
    static float slot_1[3584];
    static float slot_2[3584];
    ...
    relu(slot_1, 3584);
    // exit_conv [conv2d]
    conv2d_nhwc(slot_1, 7, 256, 2, exit_conv_weight, 3, 3, 2, NULL, 2, 2, 0, slot_0);
    // exit_bn [batchnorm]
    batchnorm2d_nhwc(slot_0, 3, 127, 2, exit_bn_gamma, exit_bn_beta, exit_bn_mean, exit_bn_var, 1e-05f, slot_1);
    // exit_relu [relu]
    relu(slot_1, 762);
    // exit_pool [adaptive_avg_pool]
    global_average_pool_2d(slot_1, 3, 127, 2, slot_0);
    // view [method_view]
    memcpy(slot_1, slot_0, 2 * sizeof(float));
    // exit_fc [linear]
    dense(slot_1, 2, exit_fc_weight, exit_fc_bias, 5, slot_0);
